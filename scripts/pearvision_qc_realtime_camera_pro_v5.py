#!/usr/bin/env python3
"""
PearVision QC - Real-Time Camera App Pro V5

Cambios respecto a V4:
  - LIVE_GOOD_ACCEPT_THRESHOLD = 0.70 (umbral PASA para camara real)
  - BAD_REJECT_THRESHOLD = 0.995 (umbral RECHAZA, igual que siempre)
  - Sin pera valida -> SIN PERA o MALA CAPTURA inmediato (nunca REVISAR)
    El smoothing se resetea de inmediato, no tras 3 ciclos.
  - Gating mejorado: candidatos con baja saturacion se rechazan como fondo.
  - Panel tecnico muestra LIVE_GOOD_ACCEPT_THRESHOLD y BAD_REJECT_THRESHOLD.
  - Banner escala el texto automaticamente para que quepa MALA CAPTURA.
  - Salida en outputs/live_camera_qc_pro_v5/.
...
Uso:
    ...
    python scripts/pearvision_qc_realtime_camera_pro_v5.py --image-folder RUTA
"""
...
MALCAP_C  = ( 20, 120, 230)   # naranja oscuro (distinto de REVISAR)
ERROR_C   = (140,  80, 180)
...
# ── Politica V5 (camara real) ─────────────────────────────────────────────────
LIVE_GOOD_ACCEPT_THRESHOLD = 0.70   # umbral PASA ajustado para camara real
BAD_REJECT_THRESHOLD       = 0.995  # umbral RECHAZA (estricto, igual que offline)
...
MIN_CANDIDATE_SAT   = 18    # media de saturacion HSV; menor => probable fondo
...
# ══════════════════════════════════════════════════════════════════════════════
#  SEGMENTACION V5 (igual que V4 + filtro saturacion)
# ══════════════════════════════════════════════════════════════════════════════
...
def _is_candidate_valid_v5(contour, frame_bgr: np.ndarray) -> tuple:
    """
    Valida contorno como candidato de pera.
    V5: anade filtro de saturacion media (rechaza fondos/sombras grises).
    ...
    # V5: rechazar si la saturacion media del ROI es muy baja (fondo/sombra)
    hsv     = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2HSV)
...
def _score_candidate_v5(metrics: dict, frame_bgr: np.ndarray) -> float:
    h, w = frame_bgr.shape[:2]
    ratio    = metrics["ratio"]
...
def _collect_candidates_v5(mask: np.ndarray, frame_bgr: np.ndarray) -> list:
    cnts, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    results = []
    for c in cnts:
        valid, metrics = _is_candidate_valid_v5(c, frame_bgr)
        if valid:
            score = _score_candidate_v5(metrics, frame_bgr)
            results.append((c, metrics, score))
    return results


def detect_pear_v5(frame_bgr: np.ndarray, bg_frame=None) -> dict:
    """
    Detecta pera con estrategia multi-mascara + mejor candidato.
    Igual que V4 pero usa _is_candidate_valid_v5 con filtro de saturacion.
    """
    h, w = frame_bgr.shape[:2]
    ...
    all_candidates += [(c, m, s, "sat")
                       for c, m, s in _collect_candidates_v5(mask_sat, frame_bgr)]
    all_candidates += [(c, m, s, "color")
                       for c, m, s in _collect_candidates_v5(mask_color, frame_bgr)]

    if bg_frame is not None:
        mask_bg = _seg_with_background(frame_bgr, bg_frame)
        all_candidates += [(c, m, s, "bg_diff")
                           for c, m, s in _collect_candidates_v5(mask_bg, frame_bgr)]
...
# ══════════════════════════════════════════════════════════════════════════════
#  PREPROCESADO PARA U3 (igual que V4 — no tocar)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
#  POLITICA DE DECISION V5
# ══════════════════════════════════════════════════════════════════════════════

def apply_policy_v5(border_cut: bool, u3_pred: str,
                    p_good: float, p_bad: float) -> tuple:
    """
    Politica V5 — solo llamada cuando mask_valid=True.
...
def draw_header(canvas, fps: float, cam_idx: int,
    ...
    _pt(canvas, "PearVision QC - Real-Time Inspection V5", 12, 34,
    ...
    info = (f"{now_str}   FPS:{fps:5.1f}   CAM {cam_idx}   "
            f"{fw}x{fh}   FRAME {frame_id:06d}   U3 thr={LIVE_GOOD_ACCEPT_THRESHOLD}")
...
def draw_tech_panel(canvas, state: dict) -> None:
    ...
    if not no_pear:
        # Marca de umbral LIVE_GOOD_ACCEPT_THRESHOLD
...
# ── Banner resultado (escala texto automaticamente) ───────────────────────────
...
def run_folder_mode(model, image_folder: Path, out_dir: Path) -> None:
    ...
    print(f"[folder] Procesando {len(images)} imagenes con politica V5")
    ...
    for img_path in images:
        ...
        if mask_valid:
            ...
            decision, reason = apply_policy_v5(border_cut, u3_pred, p_good, p_bad)
        else:
            u3_pred, p_good, p_bad = "--", 0.0, 0.0
            decision = "SIN PERA" if cap_status == "SIN_PERA" else "MALA CAPTURA"
            reason   = "NO_VALID_PEAR_DETECTED" if cap_status == "SIN_PERA" \
                       else "BAD_CAPTURE_OR_INVALID_MASK"
        t3 = time.perf_counter(); infer_ms = (t3 - t2) * 1000

        color   = DECISION_COLORS.get(decision, GRAY)
        overlay = _build_overlay(frame, det["contour"], det["bbox"], decision)

        out_path = folder_out / f"{img_path.stem}_v5_overlay.jpg"
        ...
        print(f"  {img_path.name}: {decision}  "
              f"p_good={p_good:.3f}  [{det.get('strategy_used','')}]")
    ...
    lines  = [
        "PearVision QC V5 - Folder Test Summary",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="PearVision QC Real-Time Camera App Pro V5")
    ...
    out_dir = ROOT / "outputs" / "live_camera_qc_pro_v5"
    ...
    cv2.namedWindow("PearVision QC V5", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("PearVision QC V5", CW, CH)

    saver    = EvidenceSaverV5(out_dir)
    ...
    while True:
        if not paused:
            ...
            if infer_ctr >= args.infer_every:
                ...
                det        = detect_pear_v5(frame, bg_frame)
                cap_status = det["cap_status"]
                mask_valid = det["mask_valid"]
                border_cut = det["border_cut"]
                contour    = det["contour"]
                ...
                # ── V5: sin pera valida -> reset inmediato ─────────────────
                t2 = time.perf_counter()
                if mask_valid:
                    u3_pred, p_good, p_bad = run_u3(model, gray_pil)
                    decision, reason = apply_policy_v5(
                        border_cut, u3_pred, p_good, p_bad)
                    smoother.add(decision)
                    stable      = smoother.stable()
                    no_pear_mode = False
                else:
                    u3_pred, p_good, p_bad = "--", 0.0, 0.0
                    # NUNCA dejar REVISAR cuando no hay pera valida
                    decision = ("SIN PERA" if cap_status == "SIN_PERA"
                                else "MALA CAPTURA")
                    reason   = ("NO_VALID_PEAR_DETECTED"
                                if cap_status == "SIN_PERA"
                                else "BAD_CAPTURE_OR_INVALID_MASK")
                    smoother.reset()   # reset inmediato
                    stable       = decision
                    no_pear_mode = True
                t3 = time.perf_counter(); infer_ms = (t3 - t2) * 1000
            ...
            cv2.imshow("PearVision QC V5", canvas)
        ...
        if key in (ord("q"), ord("Q"), 27):
            ...
            quick_det = detect_pear_v5(last_frame)
