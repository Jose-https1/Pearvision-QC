"""
predict_supermarket_unseen_quality_v2.py

Evaluacion del clasificador V2 (good/bad) sobre fotos reales de supermercado NO vistas.
...
Entrada:  data/unseen_quality_eval_input/
Salida:   outputs/supermarket_unseen_quality_v2_eval/
Reporte:  reports/supermarket_unseen_quality_v2_eval_report.md
"""
...
OUTPUT_DIR   = PROJECT_ROOT / "outputs" / "supermarket_unseen_quality_v2_eval"
MODEL_PATH   = PROJECT_ROOT / "outputs" / "fruits360_quality_cls_v2" / "best_model.pt"
REPORT_PATH  = PROJECT_ROOT / "reports" / "supermarket_unseen_quality_v2_eval_report.md"
...
def write_predictions_csv(out_dir: Path, rows: list):
    ...
    fields = ["image_id", "filename", "source_path", "pred_label", "pred_confidence", "prob_good", "prob_bad"]
...
def write_human_review_csv(out_dir: Path, rows: list):
    ...
    fields = ["image_id", "filename", "source_path", "pred_label", "pred_confidence", "human_label", "human_notes"]
    with path.open("w", encoding="utf-8", newline="") as f:
        w = csv.DictWriter(f, fieldnames=fields)
        w.writeheader()
        for r in rows:
...
def write_summary(out_dir: Path, rows: list, elapsed: float):
    ...
    lines = [
        "=== Supermercado Unseen V2 — Evaluacion ===",
        "",
        ...
        "",
        "Siguiente paso:",
        "  Abrir contact_sheet_all.jpg y rellenar human_review_template.csv.",
    ]
...
def write_report(rows: list, elapsed: float):
    ...
    table_rows = "\n".join(
        f"| {r['image_id']} | {r['filename']} | {r['pred_label'].upper()} | {r['pred_confidence']:.4f} | {r['prob_good']:.4f} | {r['prob_bad']:.4f} |"
        for r in rows
    )

    content = f"""# Supermarket Unseen Quality V2 — Evaluation Report

**Fecha:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
**Modelo:** `outputs/fruits360_quality_cls_v2/best_model.pt` (MobileNetV3-small, V2 congelado)
**Datos entrada:** `data/unseen_quality_eval_input/` ({len(rows)} imágenes de supermercado, NO vistas durante entrenamiento)
...
def main():
    ...
    print("=== predict_supermarket_unseen_quality_v2 ===")
    ...
    if len(img_paths) == 0:
        print("  AVISO: carpeta de entrada vacia. Copia imagenes en data/unseen_quality_eval_input/")
        write_summary(OUTPUT_DIR, [], 0.0)
        return
    ...
    make_contact_sheet(rows,      OUTPUT_DIR / "contact_sheet_all.jpg",       f"ALL — {len(rows)} imagenes supermercado")
    make_contact_sheet(good_rows, OUTPUT_DIR / "contact_sheet_pred_good.jpg", f"PRED=GOOD ({len(good_rows)})")
    make_contact_sheet(bad_rows,  OUTPUT_DIR / "contact_sheet_pred_bad.jpg",  f"PRED=BAD ({len(bad_rows)})")
    ...
    print()
    print("=" * 50)
    print("EVALUACION SUPERMERCADO V2 COMPLETADA")
    print()
    print(f"Imagenes procesadas:     {len(rows)}")
    ...
    print(f"  {REPORT_PATH}")
    print()
    print("No se entrenó ningun modelo.")
    print("No se modificó el dataset V2.")
    print("No se modificaron reglas.")
    print()
    print("Siguiente paso:")
    print("  Abrir contact_sheet_all.jpg y revisar visualmente las predicciones.")
