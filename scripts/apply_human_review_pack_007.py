"""
apply_human_review_pack_007.py
Aplica las etiquetas humanas del PACK 007 (F360_0181–F360_0210).

Modifica:  data/fruits360_human_review/human_labels_template.csv
Genera:    reports/fruits360_human_review_pack007_apply_report.md
"""
...
REPORT_MD     = PROJECT_ROOT / "reports" / "fruits360_human_review_pack007_apply_report.md"
...
EXPECTED = {"GOOD": 5, "BAD": 25, "INVALID": 0, "REVIEW": 0}
...
def write_report(labels: dict, updated: int, issues: list, prot_issues: list, rows: list) -> str:
    ...
    lines = [
        "# Fruits-360 Human Review — Aplicacion PACK 007",
        "",
        f"Fecha: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
        f"Pack aplicado: PACK 007 (IDs F360_0181 - F360_0210)",
        f"Status: **{status}**",
        "",
        "## Conteos finales PACK 007",
        "",
    ...
    lines += ["", "## Validacion PACK 007 — detalle",
              "",
              f"- IDs unicos en rango: {len(labels)}",
              f"- Rango correcto 181-210: {'SI' if all(PACK_ID_START <= int(rid.split('_')[1]) <= PACK_ID_END for rid in labels) else 'NO'}",
              f"- Sin duplicados: SI",
    ...
    lines += ["## Resumen global",
              ...
              f"- Packs completados: 001, 002, 003, 004, 005, 006, 007 (210/300)",
              "", "## Confirmaciones",
              "- NO se entrenó ningun modelo.",
              "- Solo se actualizo `human_label` para los 30 IDs del PACK 007.",
              "- PACK 001-006 no fueron modificados."]
    if all_issues:
...
def main():
    print("=== apply_human_review_pack_007 ===")
    ...
    print("  Verificando packs anteriores (001-006)...")
    ...
    print()
    print("=== RESUMEN PACK 007 ===")
    for lbl in ["GOOD", "BAD", "INVALID", "REVIEW"]:
        print(f"  {lbl:<8}: {counts.get(lbl, 0)}")
    print(f"  TOTAL   : {updated}")
    print(f"  STATUS  : {status}")
    print(f"\n  CSV total etiquetado: {total_labeled}/300")
    print(f"  Siguiente paso: revisar outputs/fruits360_human_review_packs/review_pack_008.jpg y etiquetar PACK 008 (F360_0211-F360_0240)")
