...
def _process_one(img_path: Path, seg_config: dict, rules: dict, args,
    ...
    if u3_model is not None and capture_info["capture_valid"] and u3_thresholds is not None:
        ...
        if gray_pil is None:
            ...
            strong_defect = (
                decision_before_u3 == "RECHAZA"
                or yolo_defect_metrics.get("yolo_defect_count", 0) >= 2
                or yolo_defect_metrics.get("yolo_defect_max_conf", 0.0) > 0.65
            )

            if u3_raw == "U3_BAD":
                if u3_safe_mode and not strong_defect:
                    u3_safe = "REVIEW"
                    reason = f"u3_bad_safe_mode (p_bad={p_bad:.2f})"
                else:
                    u3_safe = "BAD"
                    reason = f"u3_bad_confirmed (p_bad={p_bad:.2f})"
            ...
            # Apply U3 safe decision to pipeline decision
            if u3_safe == "BAD":
                decision = "RECHAZA"
                display_label = f"RECHAZA - U3 BAD (p_bad={p_bad:.2f})"
                metrics["display_label"] = display_label
            elif u3_safe == "REVIEW" and decision == "PASA":
                decision = "REVISAR"
